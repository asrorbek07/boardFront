...
import {useQnaBoardModify} from "~/components/qna/hooks";

export const QnaBoardModify = (
    {
        board,
        refetchBoards,
        handleClose,
  ...
  const handleMutate = async () => {
    const onSuccess = async () => {
      const response = await modifyQanBoard
        .mutateAsync(
          {
            nameValueList,
            boardId: board.id,
          ...
          {
            onSuccess: () =>
              enqueueSnackbar("Qna Board has been modified successfully", {
                variant: "success",
              }),
          }
        )
  ...
  return (
    <>
      <form onSubmit={handleSubmit(handleMutate)}>
        ...
        >
          <Typography>Modify QNA Board</Typography>
          <Button
            variant="text"
            color="primary"
        ...
        <Card sx={{ p: 1, boxShadow: "none" }}>
          ...
          >
            ...
            <LoadingButton
              loading={modifyQanBoard.isLoading}
              variant="contained"
              color="primary"
              type={"submit"}
            >
