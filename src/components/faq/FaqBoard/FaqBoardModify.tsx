...
import {useFaqBoardModify} from "~/components";
import { useSnackbar } from "notistack";
import { LoadingButton } from "@mui/lab";
import { Board, BoardCdo} from "~/models";

export const FaqBoardModify = (
    {
        board,
        refetchBoards,
        handleClose,
  ...
  const handleMutate = async () => {
    const onSuccess = async () => {
      const response = await modifyFaqBoard
        .mutateAsync(
          {
            nameValueList,
            boardId: board.id,
          ...
          {
            onSuccess: () =>
              enqueueSnackbar("Faq Board has been modified successfully", {
                variant: "success",
              }),
          }
        )
  ...
  return (
    <>
      <form onSubmit={handleSubmit(handleMutate)}>
        ...
        >
          <Typography>Modify FAQ Board</Typography>
          <Button
            variant="text"
            color="primary"
        ...
        <Card sx={{ p: 1, boxShadow: "none" }}>
          ...
          >
            ...
            <LoadingButton
              loading={modifyFaqBoard.isLoading}
              variant="contained"
              color="primary"
              type={"submit"}
            >
