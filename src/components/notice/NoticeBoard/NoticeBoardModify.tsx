...
import {useNoticeBoardModify} from "~/components/notice";

export const NoticeBoardModify = (
    {
        board,
        refetchBoards,
        handleClose,
  ...
  const handleMutate = async () => {
    const onSuccess = async () => {
      const response = await modifyNoticeBoard
        .mutateAsync(
          {
            nameValueList,
            boardId: board.id,
          ...
          {
            onSuccess: () =>
              enqueueSnackbar("Notice Board has been modified successfully", {
                variant: "success",
              }),
          }
        )
  ...
  return (
    <>
      <form onSubmit={handleSubmit(handleMutate)}>
        ...
        >
          <Typography>Modify Notice Board</Typography>
          <Button
            variant="text"
            color="primary"
        ...
        <Card sx={{ p: 1, boxShadow: "none" }}>
          ...
          >
            ...
            <LoadingButton
              loading={modifyNoticeBoard.isLoading}
              variant="contained"
              color="primary"
              type={"submit"}
            >
